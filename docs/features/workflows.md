...
- **Build metrics break at the rename** — shown on every rename. Runs recorded under
  ...
  [Build Metrics]({% link features/build-metrics.md %}) history starts fresh under the
...
| Option | What happens |
|--------|--------------|
| **Create a PR Campaign** | Opens a [PR Campaign]({% link features/pr-campaigns.md %}) whose only change is removing the workflow, tracked and merged under PR Campaigns like any other delivery. The workflow stays listed until that campaign merges. |
...
| Option | What happens |
|--------|--------------|
| **Create a PR Campaign** | Opens a [PR Campaign]({% link features/pr-campaigns.md %}) whose only change is removing the file. It appears under PR Campaigns with its pull requests grouped, so you track and merge it exactly like any other delivery. The file leaves your repositories when the campaign merges, and **Restore** cancels the removal in ActionsManager until then. |
...
| **Save & Create PR Campaign** | The workflows are added *and* a [PR Campaign]({% link features/pr-campaigns.md %}) is opened to deliver them back under ActionsManager's naming. |
...
[Reusable Workflow Project]({% link features/reusable-workflows.md %}) keeps one copy
