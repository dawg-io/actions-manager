---
title: Installation
description: Detailed installation guide for ActionsManager Self-Hosted Beta.
---
...
`ALLOW_INSECURE_HTTP=true` is required because both commands serve the app over plain HTTP: ActionsManager refuses to start on a non-loopback `http://` `APP_URL` without it, and blocks PAT login over non-local HTTP. Drop it once you put ActionsManager behind HTTPS — see [HTTPS Setup](https-setup.html).
...
See [GitHub PAT Setup](github-pat-setup.html) for token creation instructions.

### GitHub OAuth

1. Create a GitHub OAuth App (see [GitHub OAuth Setup](github-oauth-setup.html))
...
- [GitHub OAuth Setup](github-oauth-setup.html)
- [Troubleshooting](../troubleshooting/common-errors.html)
