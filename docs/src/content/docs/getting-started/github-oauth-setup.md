...
description: Configure GitHub OAuth App for browser-based GitHub login.
---
...
For the fastest single-user setup, a [Personal Access Token](github-pat-setup.html) may be simpler.
...
`ALLOW_INSECURE_HTTP=true` is required because these URLs are plain HTTP; drop it once you register HTTPS URLs and put ActionsManager behind a TLS reverse proxy — see [HTTPS Setup](https-setup.html).
...
See [GitHub PAT Setup](github-pat-setup.html) for PAT management.
