...
description: Sign in to ActionsManager using a GitHub Personal Access Token (no OAuth App required).
---
...
OAuth is also supported for browser-based GitHub login. See [GitHub OAuth Setup](github-oauth-setup.html).
...
1. Start ActionsManager (see [Installation](installation.html))
...
For a screenshot walkthrough of the login screen and the full first-run flow, see [First Workflow Walkthrough](first-workflow-walkthrough.html).
