...
description: How to put ActionsManager Self-Hosted behind HTTPS for any non-localhost deployment.
---
...
`APP_URL` derives the frontend URL, backend URL, OAuth callback, and WebSocket URL (`wss://` for an `https://` `APP_URL`) automatically — you don't need to set them individually. See [Installation](installation.html) for the full environment variable reference.
