---
title: Quick Start
description: Get ActionsManager running in under 5 minutes.
---
...
`ALLOW_INSECURE_HTTP=true` is required because this command serves the app over plain HTTP: ActionsManager refuses to start on a non-loopback `http://` address without it, and blocks PAT login over non-local HTTP. Drop it once you put ActionsManager behind HTTPS — see [HTTPS Setup](https-setup.html).
...
2. Paste a GitHub PAT with the required permissions (see [PAT Setup](github-pat-setup.html))
3. Click **Sign In**

**Option B — GitHub OAuth:**
1. Create a GitHub OAuth App and configure it (see [OAuth Setup](github-oauth-setup.html))
...
For a detailed screenshot walkthrough of every step from sign-in through saving your first local draft, see [First Workflow Walkthrough](first-workflow-walkthrough.html).

## Next Steps

- [First Workflow Walkthrough](first-workflow-walkthrough.html) — screenshot guide from PAT login to first draft save
- [Full Installation Guide](installation.html) — detailed setup options
- [GitHub PAT Setup](github-pat-setup.html) — create and configure a PAT
- [GitHub OAuth Setup](github-oauth-setup.html) — configure OAuth login
- [Projects](../features/projects.html) — understand the project model
- [Drift Detection](../features/drift-detection.html) — keep repositories in sync
