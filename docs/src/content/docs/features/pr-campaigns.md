---
title: PR Campaigns
description: Roll out workflow changes across many repositories simultaneously through coordinated pull requests.
---
...
campaign [scoped to just that repository](projects.html#adding-a-repository-to-a-project-that-already-has-workflows),
...
2. **Say whether you are done with the change** — a rollback puts the old content back in GitHub while ActionsManager still holds the new content, so it asks which one you meant. Either:
   ...
   - **Keep this change to retry later** — GitHub goes back to the previous version but ActionsManager keeps the new one, so you can fix it and deliver it again. [Drift detection](drift-detection.html) reports the rolled-back repositories until you do.
...
- [Workflows](workflows.html) — manage workflow content
- [Projects](projects.html) — organize repositories into projects
- [Drift Detection](drift-detection.html) — detect post-merge drift
