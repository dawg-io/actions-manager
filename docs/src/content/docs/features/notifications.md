---
title: Notifications
description: Get emailed when a workflow drifts or a PR campaign needs attention, instead of checking the dashboard.
---
...
- Drift notification freshness follows the drift check cadence: projects are re-checked on a background schedule (every 15 minutes by default), so a drift introduced just after a check is notified on the next one. A project whose owner has no saved GitHub token isn't checked automatically at all, and so won't notify until someone runs **Check Now** — see [Drift Detection](drift-detection.html#when-drift-is-checked).
